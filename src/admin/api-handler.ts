...
export interface AdminLogStreamOptions {
  origin?: string;
  allowOrigin?: (origin?: string) => string;
  ip?: string;
}

let tenantManagerInitialization: Promise<void> | null = null;

async function ensureTenantManagerInitialized() {
  if (!tenantManagerInitialization) {
    tenantManagerInitialization = tenantManager.initialize().catch((error) => {
      tenantManagerInitialization = null;
      throw error;
    });
  }

  try {
    await tenantManagerInitialization;
  } catch (error) {
...
export async function createAdminLogStreamResponse(
  headers: Headers,
  options: AdminLogStreamOptions = {}
  ...
  const allowOrigin = options.allowOrigin ?? ((origin?: string) => origin ?? "*");
  const origin = allowOrigin(options.origin);
