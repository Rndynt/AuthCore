# Realmio Integration Playbook for Transity & KiosKoin

This guide describes how to deploy Realmio, provision tenants, and connect the Transity and KiosKoin SaaS projects so they can rely on Realmio for identity. It combines the multi-tenant primitives provided by Realmio with the structure of each downstream application.

## 1. Prepare Realmio

1. **Set core environment variables** — `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `TRUSTED_ORIGINS`, `DATABASE_URL`, and optionally `ADMIN_API_KEY`. Use a unique secret in production because Realmio refuses to boot if `NODE_ENV=production` still uses the default development secret.【F:README.md†L27-L109】【F:src/env.ts†L1-L45】
...
Realmio ships with a tenant registry stored in `public.tenants` and a connection manager that multiplexes Prisma clients per schema.
...
| --- | --- | --- | --- | --- |
| Transity | `transity` | `transity` | `tenant_transity` | Frontend + Express backend run on the same host; set `X-Tenant-Id` header on all Realmio calls. |
| KiosKoin | `kioskoin` | `kioskoin` | `tenant_kioskoin` | Map the serverless function/project environment to this tenant when dispatching Realmio requests. |

## 3. Configure CORS and trusted origins

Include the SaaS domains and localhost ports used by Transity and KiosKoin in `TRUSTED_ORIGINS` so browser fetches with `credentials: "include"` can persist Realmio cookies. Regenerate the environment variable whenever a new preview domain is introduced.【F:src/env.ts†L17-L43】

## 4. Integrate Transity

Transity’s repository uses an Express backend (`server/index.ts`) alongside a React/TanStack Query frontend (`client/src`). The following steps attach Realmio authentication end-to-end.
...
1. **Reuse the existing `credentials: "include"` fetch wrapper** (`client/src/lib/queryClient.ts`) so Realmio session cookies flow automatically. Point Realmio requests to the deployed domain or a reverse proxy hosted alongside the Express API.
2. **Implement login & logout screens** that POST to Realmio’s `/api/auth/sign-in/email` and `/api/auth/sign-out`. Store the resolved user profile from `/me` in global state (TanStack Query or context) to hydrate the UI.【F:docs/INTEGRATION_GUIDE.md†L13-L79】【F:MULTI_TENANT_USAGE.md†L33-L70】
3. **Handle unauthorized responses** by leveraging the existing `on401` switch in `getQueryFn` to redirect users to the login page when Realmio sessions expire.
...
The `main-serverless` branch of KiosKoin combines an Express application (`server/`) with Netlify function wrappers (`netlify/functions/api.ts`). The codebase currently embeds Better Auth directly; the steps below externalise that logic so Realmio supplies all identity features.

### 5.1 Backend (Express + Netlify wrapper)

1. **Introduce Realmio environment variables** in Netlify/production and `.env` files:
   - `AUTHCORE_BASE_URL` → the public URL of the Realmio deployment (e.g., `https://authcore.example.com`).
...
2. **Replace the in-repo Better Auth instance** in `server/lib/auth.ts` with a lightweight Realmio client. Create a helper that wraps `fetch` so other modules can request Realmio data without duplicating boilerplate:
   ```ts
   ...
   ```
   Import `IncomingHttpHeaders` from `node:http` (or `http` in CommonJS builds). This keeps the Realmio integration contained in one module while exposing simple helper functions to the rest of the codebase.
3. **Proxy `/api/auth/*` traffic to Realmio**. In `server/app.ts`, remove `toNodeHandler(auth)` and wire an HTTP proxy that forwards cookies, bearer tokens, and the tenant header:
   ```ts
...
4. **Validate sessions via Realmio** in protected routes. Update the `requireAdmin` middleware in `server/routes.ts` to call `requireSession(req.headers)` (from the helper above) and perform the existing role checks on the returned user. Remove the `auth.api.getSession` usage and delete the obsolete Better Auth import.
5. **Review other Better Auth imports** such as `insertBetterAuthUserSchema` usage. For administrative flows (e.g., seeding users, bot registration), call Realmio’s admin API endpoints instead of writing directly to Better Auth tables. For example, replace direct `db.insert(user)` calls with `authcore.admin.createUser` so Realmio owns credential hashing.

### 5.2 Frontend (React client)

1. **Point the React auth client to Realmio** by rewriting `client/src/lib/auth-client.ts` to include the external base URL and tenant header:
   ```ts
...
2. **Adjust API helpers** (`client/src/lib/queryClient.ts`, any direct `fetch` calls) so every request destined for Realmio includes `credentials: "include"` and the `X-Tenant-Id` header. When calling through the Express backend, the server can inject the header, so the frontend only needs to send cookies.
3. **Update onboarding and admin flows** (e.g., pages under `client/src/pages/auth` or any admin dashboard components) to use Realmio’s endpoints for sign-in/out, password resets, and session reads. The component interfaces remain compatible because Realmio mirrors Better Auth’s REST contract.
...
- Add the Realmio environment variables to both Netlify build and runtime contexts so `server/app.ts` and static builds read them consistently.
- Redeploy after removing Better Auth migrations from `server/generate-auth-schema.ts`; the Realmio-managed schema renders those scripts unnecessary.

Following these steps migrates the existing embedded Better Auth setup to Realmio while preserving KiosKoin’s Express routing, Telegram bot flows, and Netlify deployment model.
...
- **Secret hygiene**: rotate `BETTER_AUTH_SECRET` and `ADMIN_API_KEY` regularly; Realmio already enforces a non-default secret in production builds.【F:src/env.ts†L31-L43】
...
- 2025-11-12 — Clarified Netlify admin UI routing so login requests reach the Realmio function.
